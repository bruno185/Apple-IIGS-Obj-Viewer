 ...
 * FIXED_ROUND_TO_INT
 * ------------------
 * Round a Fixed32 (16.16) value to the nearest integer, correctly handling
 * negative values. Truncation biases pixel coordinates (systematic half-pixel
 * offset) which is visually noticeable on constrained displays; this helper
 * performs nearest rounding using FIXED_HALF so that e.g. 0.5 rounds up and
 * -0.5 rounds down (towards negative infinity after adjustment), giving a
 * symmetric, unbiased conversion from fixed-point to integer coordinates.
 */
...
typedef struct {
    ...
    int auto_fit;       // 0 = manual distance, 1 = auto-fit mode (use simulated distance and scale)
    float auto_scale;   // Scale factor applied to projected coords when auto_fit is active
} ObserverParams;
 ...
 */
void drawPolygons(Model3D* model, int* vertex_count, int face_count, int vertex_count_total, ObserverParams* params);
void calculateFaceDepths(Model3D* model, Face3D* faces, int face_count);


/**
 * PAINTER'S ALGORITHM WITH NEWELL/SANCHA CORRECTIONS
 * --------------------------------------------------
 * High-level approach:
 * 1) Stable sort faces by mean depth (z_mean) descending (furthest last).
 * 2) For adjacent pairs that may overlap, perform fast rejection tests:
 *    - depth quick test (z_max[f2] >= z_min[f1] => order OK)
 *    - 2D bounding-box overlap tests (X and Y)
 * 3) For ambiguous pairs, apply Newell/Sancha plane-side tests using
 *    precomputed plane normals and the plane d term (deterministic ordering).
 * 4) Record ordered pairs to avoid oscillations; do not split polygons.
 *
 * This function produces a stable drawing order that minimizes flicker on
 * constrained hardware while avoiding costly polygon splitting.
 */
...
void getObserverParams(ObserverParams* params, Model3D* model) {
    ...
    if (fgets(input, sizeof(input), stdin) != NULL) {
        ...
        input[strcspn(input, "\n")] = 0;
        if (strlen(input) == 0) {
            // Enable auto-fit: we simulate a base distance and compute a cheap projected scale later
            if (model != NULL) {
                params->auto_fit = 1;
                params->distance = FLOAT_TO_FIXED(100.0f); // simulated distance for projection
                params->auto_scale = 1.0f;
            } else {
                params->auto_fit = 0;
                params->distance = FLOAT_TO_FIXED(100.0f);
            }
        } else {
            params->auto_fit = 0;
            params->auto_scale = 1.0f;
...
void processModelFast(Model3D* model, ObserverParams* params, const char* filename) {
    ...
    // If auto-fit is enabled, compute a cheap projected bounding scale so we can resize in drawPolygons
    if (params->auto_fit) {
        float maxAbsX = 0.0f, maxAbsY = 0.0f;
        for (i = 0; i < vtx->vertex_count; ++i) {
            if (vtx->zo[i] > 0 && vtx->x2d[i] >= 0 && vtx->y2d[i] >= 0) {
                float dx = fabsf((float)vtx->x2d[i] - (float)CENTRE_X);
                float dy = fabsf((float)vtx->y2d[i] - (float)CENTRE_Y);
                if (dx > maxAbsX) maxAbsX = dx;
        ...
        float scale = 1.0f;
        if (maxAbsX > 0.0f || maxAbsY > 0.0f) {
            float sx = (maxAbsX > 0.0f) ? (screenHalfX / maxAbsX) : 1e6f;
            float sy = (maxAbsY > 0.0f) ? (screenHalfY / maxAbsY) : 1e6f;
            scale = fminf(sx, sy);
        }
        // Clamp to reasonable bounds
        if (scale > 3.0f) scale = 3.0f;
        else if (scale < 0.2f) scale = 0.2f;
        params->auto_scale = scale;
    }
 ...
 * transformToObserver
 * --------------------
 * Transform vertices from model (object) space into observer (camera) space.
 * The transformation applies a horizontal rotation (yaw around Y), then a
 * vertical rotation (pitch around X), and finally translates along the
 * observer distance axis. Results are stored in vtx->xo, vtx->yo, vtx->zo.
 *
 * Notes on math (Fixed32, 16.16):
 *   - Uses precomputed sin/cos integer tables (degree input) for speed.
 *   - Uses 64-bit fixed multiplications (FIXED_MUL_64) for precision and
 *     to prevent overflow on intermediate products.
 *   - The chosen rotation order and signs match the original pipeline and
 *     ensure a right-handed camera coordinate system expected by projection.
 *
 * Parameters:
 *   vtx         : Vertex arrays (x,y,z input; xo,yo,zo output)
 *   angle_h_deg : horizontal angle in degrees (yaw)
 *   angle_v_deg : vertical angle in degrees (pitch)
 *   distance    : observer distance (Fixed32)
 */
void transformToObserver(VertexArrays3D* vtx, int angle_h_deg, int angle_v_deg, Fixed32 distance) {
    int i;
 ...
 * projectTo2D
 * -----------
 * Perspective projection from observer-space coordinates (xo,yo,zo) to
 * 2D screen coordinates (x2d,y2d) using fixed-point arithmetic:
 *
 * Steps per vertex (zo > 0):
 *  1) inv_zo = scale / zo                     (Fixed32 division)
 *  2) x_temp = xo * inv_zo + centre_x        (Fixed32 multiply & shift)
 *  3) y_temp = centre_y - yo * inv_zo        (screen Y axis convention)
 *  4) apply final 2D rotation by angle_w_deg  (cos/sin table)
 *  5) round the final fixed-point result to nearest integer pixel using
 *     FIXED_ROUND_TO_INT to avoid half-pixel truncation bias
 *
 * Vertices with zo <= 0 are marked invalid (-1,-1) for later culling.
 *
 * Parameters:
 *   vtx        : vertex arrays with xo,yo,zo filled
 *   angle_w_deg: final 2D screen rotation (degrees)
 */
void projectTo2D(VertexArrays3D* vtx, int angle_w_deg) {
    int i;
    Fixed32 cos_w, sin_w; 
 ...
 * drawPolygons
 * ------------
 * Render all faces in the order specified by model->faces.sorted_face_indices.
 * For each face:
 *  - Read the packed vertex index buffer to build the polygon vertex list
 *  - Skip invalid vertices (projected as -1)
 *  - Emit a QuickDraw polygon using a Pascal-style buffer (length + coords)
 *
 * The function relies on prior steps: transformToObserver/projectTo2D to
 * compute projected pixel coordinates and painter_newell_sancha to provide
 * a deterministic sorted order for faces.
 */
void drawPolygons(Model3D* model, int* vertex_count, int face_count, int vertex_count_total, ObserverParams* params) {
    int i, j;
    VertexArrays3D* vtx = &model->vertices;
        ...
        if (faces->vertex_count[face_id] >= 3) {
            ...
            for (j = 0; j < faces->vertex_count[face_id]; j++) {
                ...
                if (vertex_idx >= 0 && vertex_idx < vtx->vertex_count) {
                    int raw_x = vtx->x2d[vertex_idx];
                    int raw_y = vtx->y2d[vertex_idx];
                    int out_h, out_v;
                    if (params != NULL && params->auto_fit) {
                        float sx = (float)raw_x - (float)CENTRE_X;
                        float sy = (float)raw_y - (float)CENTRE_Y;
                        float scaled_x = (float)CENTRE_X + sx * params->auto_scale;
                        float scaled_y = (float)CENTRE_Y + sy * params->auto_scale;
                        out_h = (mode / 320) * (int)scaled_x;
                        out_v = (int)scaled_y;
                    } else {
                        out_h = (mode / 320) * raw_x;
                        out_v = raw_y;
                    }
                    poly->polyPoints[j].h = out_h;
                    poly->polyPoints[j].v = out_v;
                    if (min_x == -1 || out_h < min_x) min_x = out_h;
                    if (max_x == -1 || out_h > max_x) max_x = out_h;
                    if (min_y == -1 || out_v < min_y) min_y = out_v;
                    if (max_y == -1 || out_v > max_y) max_y = out_v;
                }
            }
    ...
    loopReDraw:
        {
            ...
            if (model->faces.face_count > 0) {
                ...
                startgraph(mode);
                // Draw 3D object
                drawPolygons(model, model->faces.vertex_count, model->faces.face_count, model->vertices.vertex_count, &params);
